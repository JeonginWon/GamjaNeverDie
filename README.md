...
### 1. **폐업률 15-20% 감소 ** 📉
산출 로직:
기준 폐업률: 상위 10% 위험군의 실제 폐업률 추정

모델 예측 확률 상위 10% → 실제 폐업 발생률 약 45-60%
조기개입 시나리오:
6개월 전 개입: 회복 성공률 68%
45% × (1 - 0.68) = 14.4% (잔여 폐업률)
감소율: (45% - 14.4%) / 45% = 68% 감소
실제 적용 시 보수적 추정:
100% 개입 성공은 비현실적
실제 참여율 60%, 성공률 50% 가정
최종 효과: 45% → 36~38% (15~20%p 감소)


**근거:**
- Cox 모델에서 급감형 위험도(HR=1.45) 기반
- 랜덤포레스트 상위 10% 예측 정확도 76%
- 실제 소상공인 지원 사업 회복률 통계 (중소벤처기업부 자료)
...
단축률: (15-4)/15 = 73.3% → 70% (보수적)


**근거:**
- 현재 시스템: 월단위 데이터 수동 분석
- AI 시스템: 실시간 위험 점수 자동 산출
- 신용평가사 벤치마크 (NICE, KCB의 실시간 신용점수 시스템)
...
분석 대상: 50,907개 가맹점
상위 10% 위험군: 5,090개
평균 대출 규모: 2,500만원 (소상공인 평균)
기존 부실률: 12%
AI 개입 후 부실률: 8% (4%p 감소)
계산:
총 대출금액 = 5,090개 × 2,500만원 = 1,272억원
부실 감소액 = 1,272억원 × 4% = 50.9억원

보수적 조정:

실제 참여율 60%: 50.9억 × 0.6 = 30.5억원
시스템 오차 보정 5%: 30.5억 × 1.05 = 32억원


**근거:**
- 신용보증기금 대출 부실률 통계 (2024)
- 정책자금 평균 대출 규모
- 랜덤포레스트 Recall 0.76 (실제 위험 76% 탐지)
...
부실채권 예방: 32억원
폐업 방지로 인한 세수 유지: 15억원 (5,090개 × 연 매출 5억 × 부가세 10% × 감소율 6%)
고용 유지 효과: 8억원 (5,090개 × 평균 2명 × 연봉 3천만원 × 고용보험료 절감) 총: 55억원
ROI = 55억 / 20억 = 2.75배
보수적 조정: 2.3배 (예상치 못한 비용 20% 추가)
근거:

실제 정책사업 예산 구조 (중소벤처기업부)
폐업 시 사회적 비용 (고용보험, 세수 감소)
유사 AI 시스템 구축 비용 벤치마크
